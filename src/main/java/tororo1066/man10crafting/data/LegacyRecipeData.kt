...
class LegacyRecipeData {
    ...
    companion object{
        fun loadFromYml(file: File, category: String, id: String): LegacyRecipeData {
//            val yml = Man10Crafting.sConfig.getConfig("recipes/${category}/$id")?:return RecipeData()
            val yml = YamlConfiguration.loadConfiguration(file)
            val recipe = LegacyRecipeData()
            recipe.key = id
            ...
            when(recipe.type){
                ...
                Type.FURNACE, Type.BLASTING, Type.SMOKING->{
                    ...
                    recipe.furnaceExp = yml.getDouble("furnace.exp").toFloat()
    ...
    fun create(){
        val namespace = NamespacedKey(Man10Crafting.plugin,key)
...
//                if (smithingTransform) {
//                    SmithingTransformRecipe(namespace, result, RecipeChoice.ExactChoice(smithingAdditionalMaterial!!), RecipeChoice.ExactChoice(singleMaterial), RecipeChoice.ExactChoice(smithingMaterial))
//                } else {
//                    SmithingTrimRecipe(namespace, result, RecipeChoice.ExactChoice(singleMaterial), RecipeChoice.ExactChoice(smithingMaterial), RecipeChoice.ExactChoice(smithingAdditionalMaterial!!), smithingCopyNbt)
//                }
                SmithingTransformRecipe(namespace,result,RecipeChoice.ExactChoice(smithingAdditionalMaterial!!),RecipeChoice.ExactChoice(singleMaterial),RecipeChoice.ExactChoice(smithingMaterial))
            }
            Type.STONECUTTING->{
    ...
    fun saveConfig(){
        if (Man10Crafting.sConfig.exists("recipes/${key}")){
            File("${Man10Crafting.plugin.dataFolder.path}/recipes/${category}/${key}.yml").delete()
        }
        val file = File("${Man10Crafting.plugin.dataFolder.path}/recipes/${category}/${key}.yml")
        if (!file.parentFile.exists()) file.parentFile.mkdirs()
        file.createNewFile()
        val config = Man10Crafting.sConfig.getConfig("recipes/${category}/${key}")!!
        ...
        Man10Crafting.sConfig.saveConfig(config,"recipes/${category}/${key}")
    }
    ...
    fun register(){
        ...
        if (type == Type.BREWING) {
            ...
            Man10Crafting.old_recipes[key] = this
            return
        }
        val namespacedKey = NamespacedKey(Man10Crafting.plugin,key)
